...
* Uses `discord.js@14` to:
  * Deny the **Speak** permission for linked members on `death`/`spectate` when they are in voice (no Discord mute chime).
  * Restore their prior permission override on `spawn`/`revive` and for every round boundary (`round_prepare`, `round_start`, `round_end`).
...
## Development Notes
* The bot never moves users between channels or toggles server deafen; it only adjusts per-user Speak overrides on the active voice channel.
* When the webhook receives round boundary events, it clears every tracked Speak override to recover from manual overrides or missed updates.
