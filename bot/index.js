...
const activeMuteOverrides = new Map(); // discordUserId -> { channelId, backup: { allow, deny } | null }
...
function getOverrideRecord(discordId) {
  return activeMuteOverrides.get(discordId) || null;
}

function setOverrideRecord(discordId, record) {
  if (record) {
    activeMuteOverrides.set(discordId, record);
  } else {
    activeMuteOverrides.delete(discordId);
  }
}
...
async function clearSpeakOverride(guild, discordId, reason, tagHint) {
  const record = getOverrideRecord(discordId);
  if (!record) {
    return false;
  }
  ...
  if (!isVoiceBasedChannel(channel)) {
    console.warn(`[Voice] Stored channel ${record.channelId} unavailable for ${tagHint || discordId}`);
    setOverrideRecord(discordId, null);
    return false;
  }
  ...
  try {
    if (record.backup) {
      await channel.permissionOverwrites.edit(discordId, {
        allow: record.backup.allow,
        deny: record.backup.deny
      }, reason);
    } else {
      const existing = channel.permissionOverwrites.resolve(discordId);
      if (existing) {
        await channel.permissionOverwrites.delete(discordId, reason);
      }
    }
    console.log(`[Voice] Restored speak permission for ${tagHint || discordId}`);
  } catch (err) {
    console.error('[Voice] Failed to restore speak permission', tagHint || discordId, err);
    return false;
  } finally {
    setOverrideRecord(discordId, null);
  }
...
async function applySpeakOverride(member, reason) {
  const voiceState = member.voice;
  if (!voiceState || !voiceState.channel) {
    console.log(`[Voice] Member ${member.user.tag} not in voice; skipping speak override`);
    return false;
  }

  const channel = voiceState.channel;
  const discordId = member.id;
  let existingRecord = getOverrideRecord(discordId);
  if (existingRecord && existingRecord.channelId !== channel.id) {
    await clearSpeakOverride(member.guild, discordId, `${reason} (channel changed)`, member.user.tag).catch(() => {});
    existingRecord = getOverrideRecord(discordId);
  }

  const overwrite = channel.permissionOverwrites.resolve(discordId);
  const speakBit = PermissionFlagsBits.Speak;
  if (!existingRecord && overwrite && overwrite.deny.has(speakBit) && !overwrite.allow.has(speakBit)) {
    console.log(`[Voice] ${member.user.tag} already denied Speak via existing override; leaving as-is`);
    return false;
  }

  const priorRecord = existingRecord ? { ...existingRecord } : null;
  if (!existingRecord) {
    setOverrideRecord(discordId, {
  ...
  try {
    await channel.permissionOverwrites.edit(discordId, { Speak: false }, reason);
    console.log(`[Voice] Denied Speak for ${member.user.tag} in ${channel.name}`);
    return true;
  } catch (err) {
    console.error('[Voice] Failed to deny Speak', member.user.tag, err);
    setOverrideRecord(discordId, priorRecord);
    return false;
  }
}
...
async function unmuteAll(reason) {
  ...
  const entries = Array.from(activeMuteOverrides.keys());
  for (const discordId of entries) {
    let member = null;
    try {
      member = await guild.members.fetch(discordId);
    } catch (err) {
      // Member might have left the guild; still attempt cleanup with stored data.
    }
    await clearSpeakOverride(guild, discordId, reason, member ? member.user.tag : discordId);
  }
}
...
async function applyMute(steamId64, shouldMute, payload) {
  ...
  const reason = `${payload.event} (${payload.round_id || 'round'})`;
  if (shouldMute) {
    await applySpeakOverride(member, reason);
  } else {
    await clearSpeakOverride(guild, member.id, reason, member.user.tag);
  }
...
client.on('messageCreate', async (message) => {
  ...
  switch (command) {
    ...
    case 'reset': {
      ...
      await message.reply('Restored Speak permission for everyone currently in voice channels.');
      break;
    }
    case 'status': {
